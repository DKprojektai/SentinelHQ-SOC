...
def _is_public_ip(ip: str) -> bool:
    """True only for routable public IPs — skips LAN/loopback so cross-agent
    correlation doesn't fire on shared private addresses (e.g. 192.168.x)."""
    try:
...
def calc_confidence(signal_type: str, severity: int, context: dict,
    ...
    # Precise match on dedicated src_ip/dst_ip columns (was: full_log LIKE,
    # which full-scanned and matched the IP in any unrelated field).
    if conn and src_ip and _is_public_ip(src_ip):
        try:
            with conn.cursor() as cur:
                cur.execute("""
