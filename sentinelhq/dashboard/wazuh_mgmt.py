...
from db import get_db, lift_decompression_limit
import wazuh_api
...
def api_wazuh_reboot():
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            # `alerts` is a compressed TimescaleDB hypertable; the DELETE below can
            # exceed the per-transaction decompression cap. Lift it for this txn.
            lift_decompression_limit(cur)
