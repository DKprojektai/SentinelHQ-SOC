...
_mitre_cache: dict = {}  # tech_id -> {name, description, tactics}


def get_mitre_technique(tech_id: str) -> dict:
    """Fetch MITRE technique info from Wazuh API. Cached in-memory."""
    tech_id = tech_id.strip().upper()
    if tech_id in _mitre_cache:
        return _mitre_cache[tech_id]
    try:
        r = requests.get(
            f"{WAZUH_URL}/mitre/techniques",
            headers=get_headers(),
            params={"q": f"external_id={tech_id}", "limit": 1},
        ...
        if items:
            item = items[0]
            tactics = []  # Wazuh stores tactic UUIDs, not names — caller passes tactic from heatmap
            url = item.get("url", "") or f"https://attack.mitre.org/techniques/{tech_id.replace('.', '/')}"
            result = {
                "id":          item.get("external_id", tech_id),
            ...
            result = {"id": tech_id, "name": tech_id, "description": "", "tactics": [], "url": ""}
        _mitre_cache[tech_id] = result
        return result
    except Exception as e:
        log.warning("MITRE technique lookup failed for %s: %s", tech_id, e)
        fallback = {"id": tech_id, "name": tech_id, "description": "", "tactics": [], "url": ""}
        _mitre_cache[tech_id] = fallback
        return fallback
