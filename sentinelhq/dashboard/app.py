...
_log = _logging.getLogger("shq.auth")
_LEGACY_SHA256_RE = _re_auth.compile(r'^[0-9a-f]{64}$')


def hash_pass(p: str) -> str:
    """Hash a password with a salted, slow KDF (pbkdf2:sha256, 600k iters)."""
    return generate_password_hash(p, method="pbkdf2:sha256:600000")


def verify_pass(p: str, stored: str) -> bool:
    """Verify a password against stored hash. Supports legacy unsalted SHA256
    (transparent upgrade happens at the call site on success)."""
    if not stored:
        return False
    if _LEGACY_SHA256_RE.match(stored):
        # Legacy unsalted SHA256 — constant-time compare
...
def is_legacy_hash(stored: str) -> bool:
    return bool(stored and _LEGACY_SHA256_RE.match(stored))
...
def _resolve_secret_key() -> str:
    """Return a strong Flask secret key.

    Priority: SECRET_KEY env (if set and not the insecure default) →
    persisted random key in the data volume → ephemeral random.
    A known/default key would let anyone forge admin session cookies.
    """
    k = os.environ.get("SECRET_KEY", "")
    if k and k != "changeme":
        return k
    _log.warning("SECRET_KEY not set or default — using a generated persistent key. "
                 "Set a unique SECRET_KEY in .env for production.")
    path = "/app/data/.secret_key"
    try:
...
# Set Secure flag when the dashboard is served over HTTPS (recommended).
app.config["SESSION_COOKIE_SECURE"] = os.environ.get("DASHBOARD_HTTPS", "false").lower() == "true"
...
_BENIGN_404_PREFIXES = ("/.well-known/", "/apple-touch-icon")
_BENIGN_404_PATHS    = {"/favicon.ico", "/favicon.svg", "/robots.txt", "/sitemap.xml"}


def _is_benign_probe(path: str) -> bool:
    return path in _BENIGN_404_PATHS or any(path.startswith(p) for p in _BENIGN_404_PREFIXES)
...
def track_http_errors(response):
    if (400 <= response.status_code < 500 and response.status_code != 401
            and not _is_benign_probe(request.path)):
        ip = _get_request_ip()
        try:
...
def ensure_admin(conn):
    ...
    if password == "changeme":
        _log.warning("DASHBOARD_PASS is the insecure default 'changeme' — "
                     "set a strong DASHBOARD_PASS in .env and change the admin password.")
...
def login_post():
    ...
    # Verify against salted hash (with legacy SHA256 fallback). Constant-time.
    if not user or not verify_pass(password, user["password_hash"]):
        record_attempt(ip, username, False, "admin")
        return redirect(url_for("login_page", error="1"))

    # Transparent upgrade: re-hash legacy SHA256 to pbkdf2 on successful login
    if is_legacy_hash(user["password_hash"]):
        try:
            with get_db() as conn:
                with conn.cursor() as cur:
                    cur.execute("UPDATE admin_users SET password_hash=%s WHERE id=%s",
                                (hash_pass(password), user["id"]))
            _log.info("Upgraded legacy password hash for user %s", username)
        except Exception as e:
            _log.warning("Password hash upgrade failed for %s: %s", username, e)
    ...
    try:
        ...
        with get_db() as conn:
            answer, new_history = ask_engine.run_agent_loop(
                question, conn, lang, history=history[-10:], fmt="markdown")
...
def api_llm_forensics_search():
    ...
    try:
        ...
        if not client_id:
            _pat = _re.compile(r'\b((?!CVE-)[A-Z]{2,}-[A-Z0-9]{4,}|\d{1,3}(?:\.\d{1,3}){3})\b')
...
def api_llm_forensics_collect():
    ...
    try:
        ...
        # Collection + LLM (param selection, live Velociraptor collect, analysis)
        # can take 1-2 min — longer than the reverse proxy timeout, which returned
        # an HTML 504 ("Unexpected token '<'"). Run in a background thread and let
        # the client poll /api/llm-forensics/result/<job_id> (each poll is instant).
        import uuid as _uuid, threading as _threading
        job_id = _uuid.uuid4().hex[:12]
        _forensic_jobs[job_id] = {"status": "running"}

        def _run_job(jid, q, cid, cname, arts, lg, hist):
            try:
                answer, new_history, _rows = velo_engine.run_selected_artifacts(
                    q, cid, cname, arts, lg, hist)
                _forensic_jobs[jid] = {"status": "done",
                                       "answer": answer,
                                       "history": (new_history or [])[-10:]}
            except Exception as e:
                log.error("forensics job %s failed: %s", jid, e)
                _forensic_jobs[jid] = {"status": "error", "error": str(e)}

        _threading.Thread(target=_run_job, daemon=True,
                          args=(job_id, question, client_id, client_name,
                                artifacts, lang, history[-10:])).start()
        return jsonify({"job_id": job_id}), 202
    except Exception as e:
        return jsonify({"error": str(e)}), 500


# In-memory store for async forensics collection jobs (job_id -> result dict)
_forensic_jobs: dict = {}


@app.route("/api/llm-forensics/result/<job_id>")
@login_required
def api_llm_forensics_result(job_id):
    """Poll endpoint for async forensics collection. Returns fast every call."""
    job = _forensic_jobs.get(job_id)
    if not job:
        return jsonify({"status": "unknown"}), 404
    if job.get("status") in ("done", "error"):
        _forensic_jobs.pop(job_id, None)   # one-shot delivery
    return jsonify(job)


@app.route("/api/llm-forensics/followup", methods=["POST"])
@login_required
def api_llm_forensics_followup():
    """Follow-up question on already-collected artifact data (no Wazuh tools)."""
    data     = request.get_json(force=True) or {}
    question = (data.get("question") or "").strip()
    history  = data.get("history") or []
    lang     = data.get("lang") or session.get("lang", "lt")
    ...
    try:
        ...
        if _tg not in sys.path:
            sys.path.insert(0, _tg)
        import velo_engine
        answer, new_history = velo_engine.run_followup(question, history[-10:], lang)
        return jsonify({"answer": answer, "history": new_history[-10:]})
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/api/otx-lookup")
@login_required
def api_otx_lookup():
    """OTX IP reputation lookup (cached)."""
    ip = request.args.get("ip", "").strip()
    if not ip:
        return jsonify({"error": "missing ip"}), 400
    try:
        return jsonify(otx_module.lookup_ip(ip))
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/api/mitre-technique")
@login_required
def api_mitre_technique():
    """Return MITRE technique details from Wazuh API (cached)."""
    tech_id = request.args.get("id", "").strip()
    if not tech_id:
        return jsonify({"error": "missing id"}), 400
    try:
        info = wazuh_api.get_mitre_technique(tech_id)
        return jsonify(info)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_mitre_heatmap():
    """Return MITRE ATT&CK technique counts grouped by tactic, last 30 days."""
    try:
        ...
        with get_db() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT mitre_id, mitre_tactic,
                           COUNT(*) AS cnt,
                           MAX(rule_level) AS max_level
                    FROM alerts
                    WHERE mitre_id IS NOT NULL
                      AND mitre_id != ''
                      AND collected_at >= NOW() - INTERVAL '1 day' * %s
                    GROUP BY mitre_id, mitre_tactic
                    ORDER BY cnt DESC
                """, (days,))
                rows = cur.fetchall()

        # Expand compound IDs like "T1070.004|T1485|T1112"
        from collections import defaultdict
        tactic_map = defaultdict(dict)  # tactic -> {tech_id: {count, max_level}}

        for r in rows:
            raw_ids    = (r["mitre_id"]     or "").split("|")
            raw_tactic = (r["mitre_tactic"] or "unknown").strip()
            count      = int(r["cnt"])
            mlevel     = int(r["max_level"] or 0)
            tactics    = [t.strip() for t in raw_tactic.split("|")] if "|" in raw_tactic else [raw_tactic]

            for i, tid in enumerate(raw_ids):
                tid = tid.strip()
                if not tid:
                    continue
                tac = tactics[i] if i < len(tactics) else tactics[-1]
                if tac not in tactic_map or tid not in tactic_map[tac]:
                    tactic_map[tac][tid] = {"count": 0, "max_level": 0}
                tactic_map[tac][tid]["count"]     += count
                tactic_map[tac][tid]["max_level"]  = max(tactic_map[tac][tid]["max_level"], mlevel)
        ...
        for tac in TACTIC_ORDER:
            if tac in tactic_map:
                seen.add(tac)
                techs = sorted(tactic_map[tac].items(), key=lambda x: -x[1]["count"])
                result.append({
                    "tactic": tac,
                    "techniques": [{"id": k, "count": v["count"], "max_level": v["max_level"]} for k, v in techs]
                })
        # Append any tactics not in TACTIC_ORDER
        for tac in sorted(tactic_map.keys()):
            if tac not in seen:
                techs = sorted(tactic_map[tac].items(), key=lambda x: -x[1]["count"])
                result.append({
                    "tactic": tac,
        ...
        return jsonify({"tactics": result, "days": days})
    except Exception as e:
        return jsonify({"error": str(e)}), 500


if __name__ == "__main__":
    # threaded=True: handle requests concurrently. Without it the single worker
    # freezes the whole UI whenever one request is slow (LLM call, Velociraptor
    # file stream, OTX lookup). Pool is a ThreadedConnectionPool so it's safe.
