...
_DEFAULT_INTERNAL_TOKEN = "sentinelhq-internal-secret"
if INTERNAL_API_TOKEN == _DEFAULT_INTERNAL_TOKEN:
    log.warning("INTERNAL_API_TOKEN is the insecure default — set a unique value in .env. "
                "Anyone able to reach the dashboard could trigger host isolation.")


def _valid_internal_token() -> bool:
    """Constant-time check of the internal service token. The default value is
    rejected so a shipped default can't be used to bypass session auth."""
    provided = request.headers.get("X-Internal-Token", "")
    if not INTERNAL_API_TOKEN or INTERNAL_API_TOKEN == _DEFAULT_INTERNAL_TOKEN:
        return False
    return hmac.compare_digest(provided, INTERNAL_API_TOKEN)


def login_required(f):
    @wraps(f)
    def decorated(*args, **kwargs):
        # Allow internal service calls with a valid (non-default) token
        if _valid_internal_token():
