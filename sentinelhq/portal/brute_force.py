...
def _notify_ip_blocked(ip: str, source: str, email: str = None):
    ...
    text = (
        ...
        f"👤 {_tr('email')}: <code>{email or '?'}</code>\n"
...
def _notify_account_locked(email: str, source: str, fail_count: int):
    token   = os.environ.get("TELEGRAM_BOT_TOKEN", "")
    chat_id = os.environ.get("TELEGRAM_CHAT_ID", "")
    if not token or not chat_id:
        return
    text = (
        f"{_tr('acc_locked')}\n──────────────\n"
        f"👤 {_tr('email')}: <code>{email}</code>\n"
        f"📍 {_tr('source')}: <b>{source}</b>\n"
        f"⚠️ {_tr('acc_fails')}: <b>{fail_count}</b> {_tr('acc_diff_ips')}\n"
        f"{_tr('acc_manual')}\n"
        f"⏰ {_local_now()}"
    )
    keyboard = {"inline_keyboard": [[{"text": _tr("unlock_btn"), "callback_data": f"unlock_account:portal:{email}"}]]}
    try:
        requests.post(f"https://api.telegram.org/bot{token}/sendMessage",
                      json={"chat_id": chat_id, "text": text, "parse_mode": "HTML", "reply_markup": keyboard},
                      timeout=10)
...
def is_blocked(ip: str, source: str = None) -> bool:
    """Jei source=None — tikrina visus portal šaltinius."""
    sources = [source] if source else ["portal", "portal_mfa", "portal_scan"]
...
def is_account_locked(email: str) -> bool:
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT locked_at FROM portal_users WHERE email=%s AND locked_at IS NOT NULL", (email,))
            return cur.fetchone() is not None


def unlock_account(email: str, actor: str) -> bool:
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                UPDATE portal_users SET locked_at=NULL, lock_reason=NULL
                WHERE email=%s AND locked_at IS NOT NULL
            """, (email,))
            affected = cur.rowcount > 0
        if affected:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM login_attempts WHERE email=%s", (email,))
            log.info("Account unlocked: %s by %s", email, actor)
        return affected
...
def get_locked_accounts() -> list:
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT email AS account, locked_at, lock_reason, 'portal' AS source
                FROM portal_users WHERE locked_at IS NOT NULL
...
def record_attempt(ip: str, email: str, success: bool, source: str):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO login_attempts(ip, email, success, source)
                VALUES(%s, %s, %s, %s)
            """, (ip, email, success, source))
        ...
        if ip_count >= MAX_ATTEMPTS and not is_blocked(ip, source):
            ...
            _notify_ip_blocked(ip, source, email)

        # --- Paskyros užrakinimas (iš visų IP) ---
        if email and not is_account_locked(email):
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT COUNT(*) AS c FROM login_attempts
                    WHERE email=%s AND success=false
                      AND source IN ('portal','portal_mfa')
            ...
            if acc_count >= ACCOUNT_MAX_FAILS:
                with conn.cursor() as cur:
                    cur.execute("""
                        UPDATE portal_users SET locked_at=NOW(), lock_reason=%s
                        WHERE email=%s
                    """, (f"{acc_count} failed attempts from multiple IPs", email))
                log.warning("Account locked: %s (%s attempts)", email, acc_count)
                _notify_account_locked(email, source, acc_count)
