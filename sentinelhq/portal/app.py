...
_log = _logging.getLogger("shq.portal.auth")
...
def _resolve_secret_key() -> str:
    ...
    if k and k != "changeme":
        return k
    _log.warning("SECRET_KEY not set/default — using generated persistent key.")
    path = "/app/data/.portal_secret_key"
    try:
...
app.config["SESSION_COOKIE_SECURE"]   = os.environ.get("PORTAL_HTTPS", "false").lower() == "true"
...
def verify_pass(p: str, stored: str) -> bool:
    ...
    if _LEGACY_SHA256_RE.match(stored):
        return _hmac.compare_digest(hashlib.sha256(p.encode()).hexdigest(), stored)
    try:
...
# Benign 404s that browsers/agents probe automatically — must NOT count as
# scanning (Chrome DevTools, favicons, well-known, etc.) to avoid self-blocking.
...
def _chrome_devtools_probe():
    # Chrome DevTools auto-requests this; answer quietly to avoid 404 noise.
    return ("", 204)
...
def portal_login_post():
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM portal_users WHERE email=%s AND is_active=true", (email,))
            user = cur.fetchone()
    ...
    # Transparent upgrade of legacy SHA256 hash on successful login
    if is_legacy_hash(user["password_hash"]):
        try:
            with get_db() as conn:
                with conn.cursor() as cur:
                    cur.execute("UPDATE portal_users SET password_hash=%s WHERE id=%s",
                                (hash_pass(password), user["id"]))
        except Exception as e:
            _log.warning("Portal password upgrade failed: %s", e)
