...
-- TimescaleDB: PK and UNIQUE must include partition column (collected_at)
ALTER TABLE alerts ADD PRIMARY KEY (id, collected_at);
CREATE UNIQUE INDEX IF NOT EXISTS alerts_wazuh_collected_key
    ON alerts(wazuh_id, collected_at) WHERE wazuh_id IS NOT NULL;

-- Convert to hypertable (7-day chunks)
SELECT create_hypertable('alerts', 'collected_at',
...
-- Compression: compress chunks older than 7 days (~70-90% size reduction)
...
ALTER TABLE alerts ADD COLUMN IF NOT EXISTS geo_country       VARCHAR(3);
ALTER TABLE alerts ADD COLUMN IF NOT EXISTS geo_asn           TEXT;
ALTER TABLE alerts ADD COLUMN IF NOT EXISTS otx_pulses        SMALLINT DEFAULT 0;
ALTER TABLE alerts ADD COLUMN IF NOT EXISTS otx_malicious     BOOLEAN DEFAULT FALSE;
ALTER TABLE alerts ADD COLUMN IF NOT EXISTS src_ip            TEXT;
ALTER TABLE alerts ADD COLUMN IF NOT EXISTS dst_ip            TEXT;
CREATE INDEX IF NOT EXISTS idx_alerts_src_ip ON alerts(src_ip) WHERE src_ip IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_alerts_dst_ip ON alerts(dst_ip) WHERE dst_ip IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS llm_analyses (
    id                 BIGSERIAL PRIMARY KEY,
    -- NOTE: no FK to alerts — it is a TimescaleDB hypertable whose PK includes the
    -- partition column, so it has no unique constraint on id alone (FK would fail:
    -- "no unique constraint matching given keys for referenced table alerts").
    alert_id           BIGINT,
...
CREATE TABLE IF NOT EXISTS telegram_messages (
    id                 BIGSERIAL PRIMARY KEY,
    alert_id           BIGINT,  -- no FK: alerts is a hypertable (see llm_analyses note)
...
-- Account-lockout columns: idempotent migration so re-running init.sql on a
-- pre-existing DB (older schema / reused volume) backfills missing columns
-- (brute_force.py needs locked_at + lock_reason on both tables).
ALTER TABLE admin_users  ADD COLUMN IF NOT EXISTS locked_at   TIMESTAMPTZ;
ALTER TABLE admin_users  ADD COLUMN IF NOT EXISTS lock_reason TEXT;
ALTER TABLE portal_users ADD COLUMN IF NOT EXISTS locked_at   TIMESTAMPTZ;
ALTER TABLE portal_users ADD COLUMN IF NOT EXISTS lock_reason TEXT;
...
CREATE TABLE IF NOT EXISTS llm_config (
    ...
    offhours_start          SMALLINT DEFAULT 20,   -- identity off-hours window start (local hour)
    offhours_end            SMALLINT DEFAULT 7,    -- off-hours window end (local hour)
    offhours_weekend        BOOLEAN DEFAULT TRUE   -- treat all weekend as off-hours
);
INSERT INTO llm_config(id) VALUES(1) ON CONFLICT DO NOTHING;

-- Idempotent column adds for existing deployments
ALTER TABLE llm_config ADD COLUMN IF NOT EXISTS offhours_start   SMALLINT DEFAULT 20;
ALTER TABLE llm_config ADD COLUMN IF NOT EXISTS offhours_end     SMALLINT DEFAULT 7;
ALTER TABLE llm_config ADD COLUMN IF NOT EXISTS offhours_weekend BOOLEAN DEFAULT TRUE;
...
-- ── System health watchdog (infra up/down state) ──────────────────────────────
CREATE TABLE IF NOT EXISTS system_health (
    component    TEXT PRIMARY KEY,            -- e.g. 'Wazuh Manager API'
    healthy      BOOLEAN NOT NULL,
    detail       TEXT,
    since        TIMESTAMPTZ DEFAULT NOW(),   -- when current state began
...
-- ── Brute force absolute signal (anksčiau naudojo default) ────────────────
('BRUTE_FORCE',            'Brute force ataka — daug autentifikacijos klaidų',             FALSE, TRUE, 10, 30),

-- ── AI / LLM-ERA grėsmės (2024-2026) ──────────────────────────────────────
('CLICKFIX_PASTE_EXEC',    'ClickFix/FileFix — netikras CAPTCHA paste-and-run',            FALSE, TRUE,  9, 30),
('CLIPBOARD_HIJACK_EXEC',  'Pastejacking — iškarpinė tiesiai į vykdymą',                   FALSE, TRUE,  9, 30),
('MCP_SERVER_ABUSE',       'Kenkėjiškas MCP serveris / tool poisoning',                    FALSE, TRUE,  8, 30),
('LOCAL_LLM_RCE',          'Vietinio LLM serverio išnaudojimas (Ollama/LM Studio)',        FALSE, TRUE,  9, 30),
('AI_MODEL_PICKLE_RCE',    'ML modelio deserializavimo RCE (pickle/torch)',                FALSE, TRUE,  9, 30),
('MALICIOUS_IDE_EXTENSION','Kenkėjiškas IDE/AI kodo plėtinys',                             FALSE, TRUE,  8, 30),
('AI_AGENT_TOOL_EXEC',     'Agentinis AI vykdo lokalias komandas',                         FALSE, TRUE,  7, 60),
('SHADOW_AI_EXFIL',        'Duomenų nutekinimas į viešus AI API (Shadow AI)',              FALSE, TRUE,  8, 30),
('GPU_CRYPTOJACK',         'GPU kriptokasimas prisidengiant AI darbu',                     FALSE, TRUE,  8, 30),
('EDR_AI_KILLER',          'Naujausi EDR/AV slopinimo įrankiai',                           TRUE,  TRUE, 12, 15),
('PKG_INSTALL_HOOK_EXEC',  'Kenkėjiškas paketo install hook (postinstall RCE)',            FALSE, TRUE,  8, 30),
('WSL_EDR_EVASION',        'WSL naudojamas Windows EDR apėjimui',                          FALSE, TRUE,  9, 30),

-- ── 2025 H2–2026 aktualios grėsmės (web intel) ────────────────────────────
('SHAREPOINT_TOOLSHELL',   'SharePoint ToolShell RCE (CVE-2025-53770/53771)',             TRUE,  TRUE, 11, 15),
('IIS_MACHINEKEY_THEFT',   'ASP.NET machineKey vagystė → ViewState forge',                FALSE, TRUE, 10, 30),
('ASPX_WEBSHELL_DROP',     'Web shell numestas web serverio procese',                     TRUE,  TRUE, 12, 15),
('BROWSER_DEBUG_THEFT',    'Naršyklės sesijų vagystė per remote-debugging',               FALSE, TRUE,  8, 30),
('COOKIE_SESSION_THEFT',   'Naršyklės cookie/login DB + DPAPI vagystė',                   FALSE, TRUE,  9, 30),
('TELEGRAM_C2_EXFIL',      'Eksfiltracija/C2 per Telegram bot API',                        FALSE, TRUE,  9, 30),
('NPM_VM_RUNTIME_EXEC',    'npm/Node stealer — runtime decrypt+vm exec',                   FALSE, TRUE,  8, 30),
('ENCRYPTIONLESS_EXTORTION','Encryptionless extortion — archyvas+cloud upload',           FALSE, TRUE, 10, 30),
('AI_POLYMORPHIC_LOADER',  'AI-polimorfinis loader — runtime LLM kvietimas',              FALSE, TRUE, 10, 30),
('VSCODE_TUNNEL_C2',       'VS Code Remote Tunnel kaip C2',                                FALSE, TRUE,  9, 30)
