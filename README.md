...
| 🤖 AI Triage | 📱 Telegram SOC | 🔗 Attack Chains | 📊 SOC Reports |
|:---:|:---:|:---:|:---:|
| Every alert analyzed by LLM in seconds | Full control from your phone — no laptop needed | Multi-step attacks detected automatically | Professional PDF reports to clients every Monday |
...
/ask what agents are connected?
→ Lists all agents — Wazuh and Velociraptor-only — with status
```

No digging. No pivot tables. No waiting. **Just ask.**

### 🔬 Deep Forensics. Live Endpoint Data.
The `/velo` command runs **live Velociraptor artifact collections** directly on endpoint machines:

```
/velo DESKTOP-B94MS7J what processes are running?
→ Searches Velociraptor's artifact library, runs collectors live, AI analyzes results

/velo HOSTNAME check for persistence mechanisms
→ Runs relevant artifacts, answers with real-time endpoint data
```

The agent context **persists across follow-up questions** — ask multiple questions about the same machine without repeating the hostname.
...
Every Monday morning, your clients automatically receive a **professional SOC-grade PDF security report** — no manual work required.

Each report includes:
- **KPI dashboard** — MTTD, MTTR, SLA compliance, False Positive Rate
- **Critical & High incident breakdown** — machine names, times, threat types
- **Actions taken** — isolations, blocks, suppressed noise rules
- **Trend comparison** — vs. previous week
- **AI-written executive summary** — clear language for management

They see what was monitored, what was detected, and what was done. You look like a pro.
...
### Weekly PDF Report — SOC-grade client report
...
- **[Wazuh 4.14.7](https://wazuh.com)** — industry-standard SIEM
