...
![SentinelHQ Banner](screenshots/banner.svg)
...
| 🤖 AI Triažas | 📱 Telegram SOC | 🔗 Atakų grandinės | 📊 SOC Ataskaitos |
|:---:|:---:|:---:|:---:|
| Kiekvienas alertas analizuojamas LLM per sekundes | Pilnas valdymas iš telefono — be nešiojamojo | Daugiažingsniai incidentai aptinkami automatiškai | Profesionalios PDF ataskaitos klientams kiekvieną pirmadienį |
...
/ask kokie agentai prijungti?
→ Rodo visus agentus — Wazuh ir Velociraptor-only — su statusu
```

Jokie kastuvavimai. Jokie suvestiniai. Jokie laukimai. **Tiesiog paklauskite.**

### 🔬 Gili forensics. Tiesioginiai galinio taško duomenys.
`/velo` komanda vykdo **gyvus Velociraptor artefaktų rinkimus** tiesiogiai ant galinio taško:

```
/velo DESKTOP-B94MS7J kokie procesai veikia?
→ Ieško Velociraptor artefaktų bibliotekoje, paleidžia kolektorius, AI analizuoja rezultatus

/velo HOSTNAME patikrink persistencijos mechanizmus
→ Paleidžia tinkamus artefaktus, atsako su realaus laiko galinio taško duomenimis
```

Agento kontekstas **išsaugomas tarp papildomų klausimų** — klausk kelis klausimus apie tą pačią mašiną nekartodamas hostname.
...
Kiekvieną pirmadienio rytą jūsų klientai automatiškai gauna **profesionalią SOC lygio PDF saugumo ataskaitą** — jokio rankinio darbo nereikia.

Kiekvienoje ataskaitoje:
- **KPI suvestinė** — MTTD, MTTR, SLA atitiktis, klaidingų signalų rodiklis
- **Kritinių ir aukšto lygio incidentų sąrašas** — kompiuterių pavadinimai, laikas, grėsmių tipai
- **Atlikti veiksmai** — izoliacija, blokavimas, nutildytos triukšmo taisyklės
- **Palyginimas su praėjusia savaite** — tendencijų rodikliai
- **AI parašyta vykdomoji santrauka** — aiški kalba vadovams

Jie mato, kas buvo stebima, kas aptikta ir kas atlikta. Jūs atrodote profesionaliai.
...
### Savaitinė PDF ataskaita — SOC lygio klientų reportas
...
- **[Wazuh 4.14.7](https://wazuh.com)** — pramonės standartas SIEM
