...
|---|---|---|
| **Wazuh Manager** | `wazuh/wazuh-manager:4.14.7` | SIEM — saugumo įvykių rinkimas |
| **Wazuh Indexer** | `wazuh/wazuh-indexer:4.14.7` | OpenSearch duomenų saugojimas (4 GB JVM heap) |
| **Wazuh Dashboard** | `wazuh/wazuh-dashboard:4.14.7` | Wazuh UI |
| **Velociraptor** | buildinamas iš šaltinio, pin `v0.76.5` | DFIR — izoliacija, forensics |
| **wazuh.logcleaner** | `alpine:3.22` | Valo rotuotus manager `.gz` logus senesnius nei 90 dienų |
| **shq-postgres** | `timescale/timescaledb:latest-pg16` | Laiko eilučių DB — alertai, koreliacijos, analizės |
| **shq-collector** | sentinelhq-collector | Alertai iš OpenSearch → TimescaleDB |
...
> Wazuh Indexer veikia su **4 GB JVM heap** (`OPENSEARCH_JAVA_OPTS=-Xms4g -Xmx4g`). Per mažas heap → OpenSearch circuit breaker atmeta vulnerability-detector rašymus → manager įstringa 100% CPU retry cikle. Laikyk ≥ 4 GB.

### Duomenų saugojimas (retention)

Nustatoma automatiškai per instaliatorių — rankiniu būdu nieko nereikia:

| Duomenys | Kur | Saugoma | Mechanizmas |
|---|---|---|---|
| Alertų indeksai | Wazuh Indexer | **90 dienų** | ISM politika `wazuh-alerts-retention` (auto `wazuh-alerts-*` / `wazuh-archives-*`) |
| Rotuoti `.gz` logai | Manager `/var/ossec/logs` | **90 dienų** | `wazuh.logcleaner` sidecar (kasdienis valymas) |
| Alertai / koreliacijos / analizės | shq-postgres (TimescaleDB) | **30 dienų** | `drop_after` retention job |

Keisti laiką: ISM `min_index_age` (Dashboard → Index Management, arba `ISM_RETENTION_DAYS` diegiant), sidecar `-mtime +90` Wazuh compose faile, DB `drop_after` politika. Pastaba: `queue/vd` (Vulnerability Detector CVE feed, ~8 GB) — operacinis duomuo, retention **nevaldomas**, netrinti dėl vietos.
...
Velociraptor diegiamas kaip atskiras Docker konteineris, buildinamas iš šaltinio ir prisegtas prie **v0.76.5** (žr. `velociraptor-docker/Dockerfile`).
...
INTERNAL_API_TOKEN=      # PRIVALOMA unikalus — python3 -c "import secrets; print(secrets.token_hex(32))"
...
# Threat intel (geo/OTX enrichment) — optional bet rekomenduojama
OTX_API_KEY=             # AlienVault OTX raktas (https://otx.alienvault.com)
...
> ⚠️ **Saugumo privalumas:** `SECRET_KEY`, `INTERNAL_API_TOKEN`, `DB_PASS`, `DASHBOARD_PASS` — kiekvienas turi būti **unikalus stiprus**. `INTERNAL_API_TOKEN` default reikšmė atmetama — **be jo auto-izoliacija neveiks**. Už TLS (reverse proxy) nustatyk `DASHBOARD_HTTPS=true` ir `PORTAL_HTTPS=true`. Slaptažodžiai saugomi `pbkdf2:sha256` (salted).
...
> ⚠️ **Visada naudok abu compose failus.** `docker-compose.override.yml` prijungia SentinelHQ konteinerius prie Wazuh Docker tinklo, leidžiant `wazuh.manager` ir `wazuh.indexer` hostname resoliuciją. Be jo collector, analyzer ir LLM agent negali pasiekti Wazuh.
...
### ❌ /velo grąžina "agentas nerastas" arba nėra artefaktų

Patikrinti:
- Hostname tiksliai atitinka Velociraptor UI rodomą (didžiosios/mažosios raidės nesvarbu)
- `sentinelhq_api.yaml` yra ir galiojantis
- Velociraptor klientas online: patikrinti `https://SERVERIO_IP:8889`
- Nurodyti hostname aiškiai: `/velo TIKSLUS-HOSTNAME <klausimas>`
...
# TELEGRAM_ALLOWED_USERS=          # optional: leidžiamų user ID sąrašas (kableliu)
...
INTERNAL_API_TOKEN=SUGENERUOTI     # PRIVALOMA unikalus
# DASHBOARD_HTTPS=true             # už TLS reverse proxy
# PORTAL_HTTPS=true
