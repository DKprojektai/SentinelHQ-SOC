...
The candidates table shows a network-IP intel column:
- **🚨 + country** (red) — IP known malicious (OTX); click → OTX details
- **country + ASN** (grey) — GeoIP location (e.g. `RU AS12345`)
- **—** — no public-IP data
...
- **Geo/OTX bar** — if the chain has a public IP: 🚨 OTX malicious / 🌍 country+ASN
- Timeline rows: 🚨OTX / ⚠️Np pulses + 🌍 flag next to the matching event
- AI explanation
- Actions: Investigate / Resolve / False Positive

> **Speed:** correlations run every 15s (was 120s). 187 attack patterns + UEBA detect attack chains near real-time.

### Telegram actions on a chain alert

When you receive a "🔴 CHAIN DETECTED" Telegram message, there are three buttons:

| Button | What it does |
|---|---|
| 🔍 **Investigate** | Opens investigation mode (no suppression) |
| ✅ **Resolved** | Closes **only THIS** incident. **Does NOT suppress** — the same chain type will alert again next time |
| 🚫 **False positive** | **Permanently mutes** this chain type for **this agent** — you won't see it again (until you unmute via `/muted`) |

> ⚠️ **Important:** to make a recurring chain GO AWAY, press **False positive**, NOT "Resolved". "Resolved" only closes one instance.

### Managing muted chains — `/muted`

The **`/muted`** command opens a button menu (no typing):

1. **Agent list** — pick an agent (the count in parentheses = how many chains muted)
2. **Signal list** — that agent's muted chains. Each row: `🔓 unmute` and `ℹ️ info`
3. **ℹ️** — shows **technical** details of the chain (so you can decide without leaving for Wazuh):
   - **🔎 Detection rules** — Wazuh rule ID + level + description
   - **🎯 MITRE ATT&CK** — technique ID + tactic (e.g. T1543.003 — Persistence)
   - **⚙️ What triggered (latest 3)** — service/process name + full path (`imagePath`) in an expandable block (tap to reveal). This shows whether it's legit (`C:\Program Files\...`) or suspicious (`%TEMP%\...`)
   - **📊 Activity** — how many times it fired, first/last date, verdict breakdown
   It also has a 🔓 Unmute button.
4. Tap a **🔓 signal** — unmutes immediately (list refreshes)
5. **⬅️ Back** — returns to the agent list

```
/muted
🔇 Muted chains — pick an agent:
[ DESKTOP-VFADT1S  (1) ]

→ after tapping the agent:
🔇 DESKTOP-VFADT1S — tap a signal to unmute:
[ 🔓 WIN_SERVICE_INSTALL ] [ ℹ️ ]
[ ⬅️ Back ]

→ after tapping ℹ️ (technical info, to decide):
ℹ️ DESKTOP-VFADT1S — WIN_SERVICE_INSTALL
🔇 Muted: 2026-06-04 14:19 (permanent)
...
⚙️ What triggered (latest 3):
• Google Updater Internal Service
  ▸ path: C:\Program Files (x86)\Google\...\updater.exe   ← expandable
    start: auto start | user mode service

📊 Activity:
Fired: 14× | first: 2026-04-11 | last: 2026-06-04
Verdicts: false_positive 7, resolved 7
[ 🔓 Unmute ]
[ ⬅️ Back ]

→ tapping 🔓 — unmuted immediately, list refreshes
```

> 💡 **How to decide:** check the `path` (imagePath). `C:\Program Files\...` + known vendor = legit → unmute confidently. `%TEMP%\`, random name = keep muted, investigate.

> You can also use the direct command: `/unmute <agent> <signal>`

> 🔒 **Security note:** "False positive" also suppresses a REAL malicious event for that agent+type. Use only when you are certain it is noise. You can re-enable anytime via `/muted` (or `/unmute`).
...
**Menu:** 🔬 LLM Forensics (or the Forensics tab in the dashboard)

The dashboard has a dedicated **LLM Forensics** panel for deep endpoint investigation via Velociraptor — the same engine as `/velo` in Telegram, but with a browser UI.

### How to Use

1. **Select an agent** — click any agent in the Agents list; the agent bar at the top of the Forensics panel shows the selected machine
2. **Ask a question** — type a forensics question in natural language
3. **Review suggested artifacts** — the system searches Velociraptor's artifact library and shows matching collectors; by default none are pre-selected
4. **Expand artifact descriptions** — click ▼ to see the full description before selecting
5. **Select artifacts to run** — check the ones you want
6. **Collect & Analyze** — the system runs the selected artifacts on the live endpoint and answers your question with the results

### Agent Context

The agent bar at the top of the Forensics panel shows which machine is currently targeted. It persists across follow-up questions — you don't need to re-select the agent for each question.
...
Playbooks are automated response rules that define what the system does when it detects a specific threat. **215 playbooks**, including the latest AI-era threats (ClickFix, MCP server, Ollama RCE, ML pickle RCE, SharePoint ToolShell CVE-2025-53770, infostealer cookie theft, Telegram C2, AI-polymorphic loader, etc.).
...
Hover over a playbook description to see a popup with:
- **MITRE** technique (e.g. T1204.004)
- **Description** (EN or LT per selected language)
- **Indicators** (processes/commands that trigger it)
...
> **🌍 Geo/OTX:** when an alert has a public IP (network connection, brute-force source), the message shows OTX threat intel and GeoIP location. Same in correlation and escalation messages.
...
| `/ask <question>` | AI analyst — ask anything about any machine (Wazuh data) |
| `/velo <question>` | LLM Forensics — deep endpoint investigation via Velociraptor artifacts |
...
| `/muted` | Muted chains (False positive) — buttons: agent → signal → 🔓/ℹ️ |
| `/help` | Help text |

### /velo — Deep Forensics via Velociraptor

The `/velo` command runs **live artifact collections** directly on endpoint machines through Velociraptor — going much deeper than Wazuh syscollector data.

**How it works:**
1. Specify the hostname: `/velo DESKTOP-B94MS7J what processes are running?`
2. The system searches Velociraptor's artifact library for relevant collectors
3. You see a list of matching artifacts — select which ones to run
4. Artifacts are executed on the live endpoint — results are analyzed by AI

**The agent context is persistent** — after you specify a hostname once, all follow-up `/velo` questions in the same session automatically target the same machine:
```
/velo DESKTOP-B94MS7J what processes are running?
→ Finds and pins the agent. Lists matching artifacts.

/velo are there any scheduled tasks?
→ Still targeting DESKTOP-B94MS7J (no need to repeat it)
```

**Works for both Wazuh+Velociraptor and Velociraptor-only agents.** If you ask `/ask kokie agentai` and see a machine tagged `[Velo]`, it means it's only in Velociraptor — use `/velo` for it.

**Example questions:**
```
/velo HOSTNAME what processes are running?
/velo HOSTNAME show me scheduled tasks
/velo HOSTNAME what network connections are active?
/velo HOSTNAME check for persistence mechanisms
/velo HOSTNAME list recently modified files
```

---

### 🔭 System health alerts (automatic)

SentinelHQ runs a **health watchdog** that every few minutes (`HEALTH_CHECK_MINUTES`, default 3) **actively knocks** on each core component and checks whether it answers. No time thresholds — a quiet period is never treated as an outage. A message is sent **only on a state change** (alive→down or down→recovered).

**6 components probed:**

| Component | What a failure means |
|---|---|
| **PostgreSQL** | The database — EVERYTHING halts |
| **Wazuh Manager API** | Isolation and rule management via API stop |
| **Wazuh Indexer** | Alert collection STOPS (collector reads from here) |
| **Velociraptor** | Forensics and isolation won't work |
| **LLM API** | AI verdicts and correlation analysis paused (alerts still collected) |
| **OTX** | Non-critical — IP reputation enrichment skipped |
...
cluster=red
🛠 What to do: Alert collection STOPS (collector reads from here).
Common cause: full disk or low heap. Check the indexer container;
cluster may be 'red'.
⏰ 2026-05-31 11:50
```
...
✅ RECOVERED
──────────────
Wazuh Indexer is back (2026-05-31 11:50 → now)
⏰ 2026-05-31 12:05
```

Every down alert includes a **🛠 What to do** line with the impact and first action — so you know how to react even without deep system knowledge.

> **Internal services** (collector, analyzer, llm-agent) are not time-monitored separately — Docker auto-restarts them on crash, and their real failures surface through one of the 6 components above (e.g. Indexer or LLM).

---

### 🧭 How to ask correctly (`/ask` and `/velo`)

Shared rules for both commands so the system recognizes the machine:

| Input | Meaning |
|-------|---------|
| `/ask kali what processes` | **First word = machine** (hostname/IP). The rest is the question. |
| `/ask #685574 what happened` | **`#` = incident** (alert/correlation ID). |
| `what users` (no command) | **Continues the same machine** — no need to repeat the name. |
| Different machine | Use `/ask` or `/velo` again with a new first word. |
| `/ask which machine has user jdoe` | **Cross-machine question** — first word is not a machine. |

**Example (session):**
```
/ask kali what processes are running?       → answers about kali, remembers kali
what users are logged in?                   → still about kali (no command)
/ask WIN-23JJCFPQEPF scheduled tasks?        → switches to another machine
```

`/velo` works the same: `/velo kali what processes` or `/velo #685574 ...`.

> **Note:** `/ask` uses Wazuh data; `/velo` does live Velociraptor collection. The machine must be known (a Wazuh agent or a Velociraptor client).
...
**Tip:** `/ask` covers Wazuh-enrolled agents. For machines only in Velociraptor (tagged `[Velo]` in agent list), use `/velo` instead.
...
/ask what agents are connected?             (shows both Wazuh and Velociraptor-only agents)
/ask lookup IP 185.220.101.5               (IP reputation + geolocation + threat intel)
```
...
| 💬 **Ask AI** | **Technical** alert analysis — full command line, parent chain, hashes, MITRE sub-technique, mechanism (for advanced users; not generic advice) |
...
**PDF sections:**
- **KPI** — confirmed incidents, isolations, monitored machines, MTTD, SLA, noise %
- **Alert Statistics** + **Incident Status**
- **Fleet Overview** — alerts/high/confirmed per machine (whole fleet)
- **Notable Threats** — deduplicated by machine + type
- **Response Summary** — isolations, unblocks, suppress rules (counts)
- **Security Posture (live Wazuh data):**
  - **Asset Coverage** — agents active/disconnected/total + offline list
  - **Vulnerabilities (CVE)** — Critical/High/Medium/Low + machines with most CVEs
  - **Configuration Hardening (CIS)** — CIS hardening score per machine (worst 10; with offline flag + scan date)
  - **Exposed Risky Ports** — externally-listening (SMB, RDP, DBs, etc.): `port/proto (process)` (top 10 riskiest machines)
- **Summary** — AI narrative in business terms

> Security-posture data is pulled live from the Wazuh API + indexer. If Wazuh is unreachable, those sections are skipped and the report is still generated.
