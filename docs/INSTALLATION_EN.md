...
|---|---|---|
| **Wazuh Manager** | `wazuh/wazuh-manager:4.14.7` | SIEM — security event collection |
| **Wazuh Indexer** | `wazuh/wazuh-indexer:4.14.7` | OpenSearch data storage (4 GB JVM heap) |
| **Wazuh Dashboard** | `wazuh/wazuh-dashboard:4.14.7` | Wazuh UI |
| **Velociraptor** | built from source, pinned `v0.76.5` | DFIR — isolation, forensics |
| **wazuh.logcleaner** | `alpine:3.22` | Prunes rotated manager `.gz` logs older than 90 days |
| **shq-postgres** | `timescale/timescaledb:latest-pg16` | Time-series DB — alerts, correlations, analyses |
| **shq-collector** | sentinelhq-collector | Alerts from OpenSearch → TimescaleDB |
...
> The Wazuh Indexer runs with a **4 GB JVM heap** (`OPENSEARCH_JAVA_OPTS=-Xms4g -Xmx4g`). A too-small heap makes the OpenSearch circuit breaker reject vulnerability-detector writes, which pins the manager at 100% CPU in a retry loop. Keep ≥ 4 GB.

### Data retention

Set up automatically by the installer — no manual action needed:

| Data | Where | Retained | Mechanism |
|---|---|---|---|
| Alert indices | Wazuh Indexer | **90 days** | ISM policy `wazuh-alerts-retention` (auto-applies to `wazuh-alerts-*` / `wazuh-archives-*`) |
| Rotated `.gz` logs | Manager `/var/ossec/logs` | **90 days** | `wazuh.logcleaner` sidecar (daily prune) |
| Alerts / correlations / analyses | shq-postgres (TimescaleDB) | **30 days** | `drop_after` retention job |

To change a period: ISM `min_index_age` (Dashboard → Index Management, or `ISM_RETENTION_DAYS` at install), the sidecar `-mtime +90` in the Wazuh compose, and the DB `drop_after` policy. Note: `queue/vd` (Vulnerability Detector CVE feed, ~8 GB) is operational data — it is **not** retention-managed and should not be deleted for space.
...
Velociraptor is deployed as a separate Docker container, built from source and pinned to **v0.76.5** (see `velociraptor-docker/Dockerfile`).
...
INTERNAL_API_TOKEN=    # REQUIRED, unique — python3 -c "import secrets; print(secrets.token_hex(32))"
...
# Threat intel (geo/OTX enrichment) — optional but recommended
OTX_API_KEY=           # AlienVault OTX key (https://otx.alienvault.com)
...
> ⚠️ **Security requirement:** `SECRET_KEY`, `INTERNAL_API_TOKEN`, `DB_PASS`, `DASHBOARD_PASS` must each be **unique and strong**. The default `INTERNAL_API_TOKEN` is rejected — **auto-isolation will not work without it**. Behind TLS (reverse proxy) set `DASHBOARD_HTTPS=true` and `PORTAL_HTTPS=true`. Passwords are stored as `pbkdf2:sha256` (salted).
...
> ⚠️ **Always use both compose files.** `docker-compose.override.yml` attaches SentinelHQ containers to the Wazuh Docker network, enabling `wazuh.manager` and `wazuh.indexer` hostname resolution. Without it, the collector, analyzer, and LLM agent cannot reach Wazuh.
...
```bash
docker compose -f docker-compose.yml -f docker-compose.override.yml logs -f shq-collector
docker compose -f docker-compose.yml -f docker-compose.override.yml logs -f shq-analyzer
docker compose -f docker-compose.yml -f docker-compose.override.yml logs -f shq-llm
docker compose -f docker-compose.yml -f docker-compose.override.yml logs -f shq-telegram
```
...
### ❌ /velo returns "agent not found" or no artifacts

Check:
- The hostname matches exactly what appears in Velociraptor UI (case-insensitive)
- `sentinelhq_api.yaml` is present and valid
- Velociraptor client is online: check `https://SERVER_IP:8889`
- Try specifying the hostname explicitly: `/velo EXACT-HOSTNAME <question>`
...
LLM_MODEL=google/gemini-2.5-flash
LLM_POLL_INTERVAL=15
LLM_CRITICAL_BACKLOG_HOURS=48

TELEGRAM_BOT_TOKEN=CHANGE_ME
TELEGRAM_CHAT_ID=CHANGE_ME
# TELEGRAM_ALLOWED_USERS=          # optional: comma-separated allowed user IDs

OTX_API_KEY=                       # AlienVault OTX (geo/threat-intel enrichment)
OTX_MALICIOUS_MIN_PULSES=2

DASHBOARD_PORT=8082
DASHBOARD_USER=admin
DASHBOARD_PASS=CHANGE_ME
SECRET_KEY=GENERATE_ME
INTERNAL_API_TOKEN=GENERATE_ME     # REQUIRED, unique
# DASHBOARD_HTTPS=true             # behind TLS reverse proxy
# PORTAL_HTTPS=true
